...
export class NavigationsComponent implements OnInit{   
  ...
  constructor(
   ...
   {
      var menu: Menu;
      let links: Link[] = routes.map((route) => {
         let link: Link;
         link.path = route.path;
         link.data.name = route.data.name;
         link.data.icon = route.data.icon;
         link.data.type = route.data.type;
         return link;
      });
      console.log(links);
      let pages = links.reduce((acc, value) => {
            console.log(value);
            // if(value.data.type) acc.public.push(value);
