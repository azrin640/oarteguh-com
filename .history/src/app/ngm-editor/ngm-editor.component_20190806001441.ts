...
export class NgmEditorComponent implements OnInit {
   ...
   createNode(cmdButton: MatButton){  
      ...
      selected = this.createElement(selected, command);
      let newElement = selected.newElement;
   ...
   contentSelected(): Selected
   {
      ...
      // Slice initial content string to only selected string and return content object
      let initContent = this.content;      
      let content = '';

      if(selected.start > selected.end) content = initContent.slice((selected.end), (selected.start))
      else content = initContent.slice((selected.start), (selected.end));
   ...
   createElement(selected, command): Selected
   {     
