...
export class NgmEditorComponent implements OnInit {
   ...
   {           
         ...
         if(selected && selected.type === 'caret'){  
            ...
            // let anchorParentNode = selection.anchorOffset;
            // let focusParentNode = selection.collapseToEnd();                  
            let range: Range = selected.ranges[0];
            console.log(range);
            // let parentNode = range.commonAncestorContainer;
            // let startNewNode = selection.anchorOffset;
            // range.insertNode(newElement);
            // range.setStart(parentNode, startNewNode);
