...
export class NgmEditorComponent implements OnInit {
   ...
   contentSelected(): Selected
   {
      ...
      this.ranges = ranges;
      let length = selection.focusNode.nodeValue.length;
      ...
      let content = '';
      if(selected.start > selected.end) content = selectedContent.slice((selected.end), (selected.start))
      else content = selectedContent.slice((selected.start), (selected.end)) 
   ...
   createNode(inputA, inputB){
         ...
         if(inputB === 'dummy'){ 
            cmdButton = inputA._elementRef.nativeElement.id;
            selected = this.contentSelected();            
            let ranges = this.ranges;
            var range = ranges[0];            
         }
         else{
            var caretPosition = document.caretPositionFromPoint(0, 0);
            console.log(caretPosition);
            cmdButton = inputA;
            let target = inputB.target;
            console.log(target);
         }
