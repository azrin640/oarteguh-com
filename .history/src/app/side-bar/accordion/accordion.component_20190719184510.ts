...
export class AccordionComponent implements OnInit {
   ...
   formProfile;
   formList;
   fG =  { pI: [  { name: {   type: 'text', 
                              formCtrlName: 'name', 
                              placeHolder: "{{ profile.name || 'Name' | titlecase }}",
                              action: "updateProfile({ _id: profile._id, name: nameCtrl.value, type: 'string' })",
                              getter: 'nameCtrl',
                              iconColor:  "nameCtrl.invalid ? 'warn' : 'primary'",
  ...
  get birthday(){ return this.formProfile.get('birthday')};
