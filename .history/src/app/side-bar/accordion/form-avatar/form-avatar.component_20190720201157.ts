...
export class FormAvatarComponent implements OnInit {
  ...
  updateProfile(userData)
   {  console.log(userData);
      ...
      let slicedItemValue = itemValue.split('/');
      console.log(slicedItemValue[1]);

      this._profileService.updateUser(userData).subscribe(
         (response: User) => {
            if(response && response._id){
               this._snackBar.open(`Your " ${itemKey} " update to " ${slicedItemValue} " is successfull`, 'X', {duration: 10000, panelClass: 'pink-style'});
