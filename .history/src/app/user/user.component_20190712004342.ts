...
export class UserComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   var pObj = {category: '', posts: []};
   let newPosts = posts.map((p) => {
      let obj = newCtgs.reduce((acc, value) => {
         if(p.post.category === value){
            acc = { category: value, posts: [p]};
         }
         return acc;
      }, {});
      let newObj = Object.assign({category: obj.category}, obj);
      return newObj;
   });
