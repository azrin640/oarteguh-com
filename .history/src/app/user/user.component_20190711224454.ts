  ...
  ngOnInit() {
   ...
   let p = posts
      .reduce((acc, value, index, array) => {
         arr.push(value.post.category);

         let arr2 = arr.reduce((acc2, value2, index, array) => {
            if(value.post.category === value2 ) acc2.push(value);
            return acc2;
         }, []);

         // acc = [{ category: value.post.category, posts: arr2}]
         acc = arr2;
         return acc;
      }, []);
   ...
   // .reduce((acc, value) => {  
   //    let obj = {category: value.post.category, posts: [{ post: { ref: value.post, poster: value.poster }}]};
   //    acc.push(obj); 
   //    return acc;
   // }, [])
   // .reduce((acc, value, index, array) => {
   //    let exist = acc.find(value.category);
   //    if(!exist) acc.push(value);
   //    else if(exist) acc.posts.push()
   //    acc = index;
   //    return acc;
   // }, [])
