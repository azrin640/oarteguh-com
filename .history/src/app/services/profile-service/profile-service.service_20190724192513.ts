...
export class ProfileService {
  ...
  constructor( 
      ...
      return this.http.post("/api/user/login", credentials).subscribe(
         (response: User) => {
            if(response && response.id){
               ...
               this.snackBar.open('Congratulation, your login is successful.', 'X', {duration: 10000, panelClass: 'pink-theme'});
   ...
   {
      return this.http.post('/api/user/profile/update/autocomplete/city/state-country', city).pipe(
         catchError(error => throwError(error))
      );
   }
   ...
   updateUser(userData)
   {  return this.http.post('/api/user/profile/updat', userData, httpOptions).pipe(
         catchError(error => throwError(error))
      );
   }
